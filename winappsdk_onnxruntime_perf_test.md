...
A set of convenience batch scripts lives under `chrisd/` (and a couple at the
repo root). They wrap the configure / build / run commands above for the
common scenarios. They assume you run them from the repo root in a
...
| `go.cmd` | Two NVIDIA TensorRT RTX runs — one default, one with `--winappsdk_register_provider NvTensorRTRTXExecutionProvider` to demonstrate selective registration. |

### Misc

* `chrisd\go-cmake-logs.cmd` — re-run CMake configure with logs piped to a
  file for triage.
* `copy-perf-test.cmd [BuildType] [Arch]` — copies the built
