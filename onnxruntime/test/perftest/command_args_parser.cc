...
ABSL_FLAG(std::string, e, "cpu", "Specifies the provider 'cpu','cuda','dnnl','tensorrt', 'nvtensorrtrtx', 'openvino', 'dml', 'acl', 'nnapi', 'coreml', 'qnn', 'snpe', 'rocm', 'migraphx', 'xnnpack', 'vitisai', 'webgpu' or 'amdgpu'.");
#else
...
bool CommandLineParser::ParseArguments(PerformanceTestConfig& test_config, int argc, ORTCHAR_T* argv[]) {
  ...
  // -e
  {
    ...
    if (!ep.empty()) {
      if (ep == "cpu") {
        ...
        test_config.machine_config.provider_type_name = onnxruntime::kAMDGPUExecutionProvider;
#endif
      } else {
        return false;
      }
  ...
  // -es
  test_config.run_config.f_enable_extended_stats = absl::GetFlag(FLAGS_es);
