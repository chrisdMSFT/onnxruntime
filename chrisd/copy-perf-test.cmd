...
REM (Microsoft.Windows.AI.MachineLearning.dll, onnxruntime.dll,
REM onnxruntime_providers_shared.dll, DirectML.dll)
...
REM Anchor source to the repo root (parent of this script's folder) so the
REM script works regardless of the current working directory.
set "RepoRoot=%~dp0.."
set "SourceDir=!RepoRoot!\build\!BuildType!"
...
for /f "tokens=*" %%a in ('git -C "!RepoRoot!" rev-parse --short HEAD') do set Commit=%%a
...
REM Get the current branch name (sanitize any '/' to '-' for path safety)
for /f "tokens=*" %%a in ('git -C "!RepoRoot!" rev-parse --abbrev-ref HEAD') do set Branch=%%a
set Branch=!Branch:/=-!
...
set DestFolder=!DestBase!\!Branch!\!FolderName!\!Arch!
...
set OrtProvidersDllPath=!SourceDir!\onnxruntime_providers_shared.dll
...
if not exist "!OrtProvidersDllPath!" (
    echo Error: DLL not found: !OrtProvidersDllPath!
    exit /b 1
)
...
copy "!OrtProvidersDllPath!" "!DestFolder!\" /Y
if errorlevel 1 (
    echo Error: Failed to copy onnxruntime_providers_shared.dll
    exit /b 1
)
echo Copied: !OrtProvidersDllPath! -^> !DestFolder!
...
echo   - onnxruntime_providers_shared.dll
echo   - DirectML.dll
echo.
echo Success!
